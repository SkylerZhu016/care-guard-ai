...
- 系统以桌面端教学演示为主，同时保证 390px 级移动视口可以完成登录、患者填报、人工审核和任务执行的核心流程；生产级高并发与真实医院接入不在目标内。
...
面向有限症状范围的基层医疗教学协作系统：模拟患者从受控目录提交结构化信息，确定性规则首先识别已配置红旗风险，AI 在公开指南范围内完成信息整理、证据检索和风险复核，医务人员人工终审后再明确决定“不创建随访”或选择适用模板，全流程保留版本、引用、安全告警和审计记录。
...
- 预问诊自动规则范围固定为 `LOCAL_SYMPTOM_V1` 的 4 个受控症状：`CHEST_PAIN`（胸痛）、`DYSPNEA`（呼吸困难）、`SYNCOPE`（晕厥）和 `ALTERED_CONSCIOUSNESS`（意识异常）。患者只通过中文目录选择，内部代码由系统维护。
- 自由文本只用于补充合成教学场景，不能被解释为对目录外症状完成了自动安全评估。目录外症状必须被明确标为“未配置自动规则/需人工审核”，不得静默显示为常规或无风险。
- AI 只输出结构化病例摘要、建议的就医紧急度、待补充问题和指南证据，不输出确定诊断、处方、剂量或替代医生的行动指令。
- 随访 MVP 只实现一个“高血压教学随访”模板。它是可选能力而非审核后的默认下一步：只有明确标记为高血压教学场景的病例才可选择；其余病例必须选择“不创建随访”。扩展其他路径和服务端模板适配规则属于后续范围。
...
- 人工终审与随访选择是两个独立决定；默认值必须是“不创建”，不能因审核通过而自动创建、预选或激活高血压模板。
- 选择模板前必须校验病例适用范围；前端隐藏/禁用不适用模板，后端仍是最终校验者。
...
高血压教学随访模板 v1 只定义工程流程，不定义治疗方案，也不适用于普通胸痛、呼吸困难等病例。只有医务人员对明确的高血压教学病例显式选择并激活后，才生成以下任务：
...
- 登录/教学用途确认。
- 合成患者档案。
- 默认空状态的三步预问诊：合成主诉、中文受控症状目录、确认提交；红旗案例只能通过明确的“加载教学示例”动作载入。
- 规则覆盖提示：明确列出 4 个受控症状，并说明“未命中当前规则”仍需人工审核。
- 提交结果和待审核状态。
- 自己的随访任务列表。
...
- 审核表单：接受/修改/拒绝和必填理由。
- 独立随访决策：默认“不创建”，仅对明确适用病例开放高血压教学模板，再单独确认激活。

随访人员：

- 今日/逾期任务看板。
- 任务详情和结果登记。
- 清楚显示任务来自医务人员明确激活的模板，不把所有任务笼统描述为高血压任务。
...
- 用户与角色。
- 指南版本、入库状态和知识库激活。
- 知识来源提供搜索、发布方筛选和分页；许可、哈希、对象路径等技术字段默认折叠。
...
- 患者不得手工输入或维护内部症状代码；显示名称、同义词和覆盖状态来自受控目录。
- 页面声明的能力边界必须与已激活规则目录一致；`NO_CONFIGURED_RED_FLAG` 只能解释为“未命中当前已配置红旗，仍需人工审核”，不得写成“安全”或“无风险”。
- 终审与随访是两个独立步骤；随访默认“不创建”，模板参数必须显式提交，API 客户端不得设置高血压默认值。
- 长队列和知识来源列表必须提供信息收敛、筛选或分页；哈希、对象路径等技术噪声按需展开。
- 动态新增、删除、验证、保存和任务状态区域必须具备可访问名称，关键变化使用 `aria-live` 或等效通知。
- 核心流程必须在 1440×1000 桌面视口与 390×844 移动视口可完成，并保留可见键盘焦点。
...
- 合成患者、默认空状态分步表单、4 项中文受控症状目录、草稿和提交。
- Visit 状态机、版本控制和幂等提交。
...
- 接受、修改、拒绝 AI 草案并记录理由。
- 最终风险结果和审计记录。
- 人工终审与随访决策分离，默认“不创建”；高血压教学模板仅对明确适用病例可选，并覆盖计划状态机、任务生成、逾期和完成。
- 随访人员工作台与患者查看任务页面。

完成定义：患者填报 -> 规则 -> RAG/Agent -> 医务人员审核 -> 明确“不创建”并关闭，或对适用病例显式选择模板 -> 激活计划 -> 完成任务 -> 归档，两条分支均通过。
...
6. 医务人员修改或接受草案并填写审核理由；先展示默认“不创建”的安全分支。
7. 切换到一个明确标记的高血压教学病例，显式选择并激活高血压模板；随访人员完成一项生成任务，模拟患者看到自己的状态。
