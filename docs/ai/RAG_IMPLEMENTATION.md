...
完整工程链路为：固定官方来源白名单 → 检查 robots.txt → 限速、重试并抓取正文 → 清理页面模板和重复段落 → 稳定分块 → 生成带 SHA-256 的 JSON 快照 → 后端校验哈希 → Markdown 原文上传 MinIO → 来源、版本和抓取证据写入 PostgreSQL → 64 维确定性 hashing embedding 写入 pgvector → 症状主题精确命中、分块症状词匹配、cosine 相似度依次排序的混合检索 → 同一来源最多保留 2 个分块 → Agent 引用白名单校验。
