...
export interface SymptomReport { id?:string; symptomCode: string; name?: string; customName?:string; source: 'CATALOG'|'RELATED_ANSWER'|'CUSTOM'|'LEGACY'; catalogVersion?:string; supportLevel?:SupportLevel; onsetRange?:string; course?:string; currentStatus?:string; activityImpact?:string; answers:QuestionAnswer[]; legacySeverity?:number; legacyOnset?:string }
export interface Visit { id: string; ownerId: string; status: string; intakeVersion:string; primarySymptomCode?:string; chiefComplaint: string; freeText: string; symptomReports: SymptomReport[]; profileSnapshot?:PatientProfileInput; triage?: Triage; runs: Run[]; supplements:VisitSupplement[]; createdAt: string; submittedAt?: string }
