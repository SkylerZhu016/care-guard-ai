...
async def triage(input_data: PreConsultInput):
    """Perform AI triage analysis on pre-consultation data."""
    try:
...
async def submit_consult(input_data: PreConsultInput):
    """Full consultation pipeline: triage + safety check."""
    run_id = str(uuid.uuid4())
    start = time.time()

    # Step 1: Safety check input
    ...
    # Step 2: Triage
    triage_result = engine.triage(input_data)

    # Step 3: Overall safety
